...
const ProfileUI = {
  render() {
    ...
    const finished = [...movies.map(m=>({...m,mediaType:'movie'})),...tvshows.map(t=>({...t,mediaType:'tv'}))]
      .filter(i=>i.watchStatus==='completed')
      .sort((a,b)=>new Date(b.endDate||b.dateUpdated||b.dateAdded)-new Date(a.endDate||a.dateUpdated||a.dateAdded)).slice(0,5);
    ...
    page.innerHTML = `<div class="profile-container"><div class="page-header"><h1>Profile</h1></div>
      ...
      <div class="profile-section"><h3>Recently Finished</h3>${finished.length?`<div class="recent-row">${finished.map(item=>{const poster=TMDB.poster(item.posterPath,'w185');const ur=Store.getUserRating(item.tmdbId,item.mediaType);return`<div class="recent-card" data-tmdb="${item.tmdbId}" data-type="${item.mediaType}"><div class="recent-poster-wrap">${poster?`<img src="${poster}" loading="lazy">`:`<div class="no-poster-ph" style="width:100%;height:100%;font-size:10px;">${item.mediaType==='movie'?'MOV':'TV'}</div>`}<div class="poster-overlay"></div><div class="poster-badge badge-completed">Completed</div>${ur?`<div class="recent-user-rating">★ ${ur}</div>`:''}</div><div class="recent-info"><div class="recent-title">${esc(item.title)}</div><div class="recent-meta">${item.mediaType==='movie'?'Movie':'TV'} · ${item.year||''}</div></div></div>`;}).join('')}</div>`:'<div class="empty-inline">No completed titles yet</div>'}</div>
